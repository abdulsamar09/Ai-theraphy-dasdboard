...
class SessionService {
    ...
    async bootstrap() {
        const urlParams = new URLSearchParams(window.location.search);
        const sid = urlParams.get("sid") || urlParams.get("join") || sessionStorage.getItem('active_session_id');
        
        let url = `${API_BASE}/api/dashboard/bootstrap`;
        if (sid) {
            url += `?room_id=${encodeURIComponent(sid)}`;
        }
