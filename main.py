...
SMTP_PORT = 465
SMTP_USER = "jonkogen@aol.com"
SMTP_PASSWORD = "bb363636"
...
async def signup(signup_data: SignupRequest, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    if signup_data.role == "therapist":
        profile = TherapistProfile(user_id=new_user.id, license_data=signup_data.license_data, state_of_licensure=signup_data.state, approval_status="pending", degree=signup_data.degree, license_type=signup_data.license_type, license_number=signup_data.license_number)
        db.add(profile)
    
    # Create wallet with 20 minute free session
    wallet = Wallet(user_id=new_user.id, minutes_remaining=20.0)
    db.add(wallet)
    db.commit()
